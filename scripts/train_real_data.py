...
def run_resume_training_real(cfg: DictConfig, checkpoint_path: str, start_epoch: int = 0) -> DDPMTrainer:
    """Resume training routine for real microscopy data from checkpoint with adaptive learning."""
    ...
    if cfg.wandb.mode != "disabled":
        wandb.init(
            ...
            entity=cfg.wandb.entity,
            config=OmegaConf.to_container(cfg, resolve=True),
            name=f"{cfg.experiment.name}_resume_epoch_{start_epoch}",
        )
    ...
    # Create trainer module with updated config for adaptive learning
    trainer_config = OmegaConf.to_container(cfg.training, resolve=True)
    # Ensure adaptive learning is enabled
    trainer_config["use_scheduler"] = True
    ddpm_trainer = DDPMTrainer(
        model=unet,
    ...
    # Optimizer and scheduler (with adaptive learning enabled)
    optim_or_pair: Union[torch.optim.Optimizer, Tuple] = ddpm_trainer.configure_optimizers()
    if isinstance(optim_or_pair, tuple) or isinstance(optim_or_pair, list):
        if isinstance(optim_or_pair[0], list):
            optimizer = optim_or_pair[0][0]
    ...
    early_stopping_patience = int(getattr(cfg.training, "early_stopping_patience", 20))
    early_stopping_counter = 0
    min_delta = float(getattr(cfg.training, "early_stopping_min_delta", 0.0001))
    ...
    print(f"🚀 Resuming training with ADAPTIVE LEARNING RATE and EARLY STOPPING")
    print(f"📊 Starting from epoch {start_epoch + 1}, target epochs: {max_epochs}")
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        for batch_idx, batch in enumerate(progress):
            ...
            writer.add_scalar("train/loss", float(loss.detach().item()), global_step)
            writer.add_scalar("train/lr", float(optimizer.param_groups[0]["lr"]), global_step)
            progress.set_postfix({"loss": f"{float(loss.detach().item()):.4f}", "lr": f"{optimizer.param_groups[0]['lr']:.2e}"})
        ...
        if avg_val_loss < best_val_loss - min_delta:
            ...
            # Save best optimizer, scheduler, and scaler states
            try:
                torch.save(optimizer.state_dict(), f"{checkpoint_dir}/best_optimizer.pt")
            except Exception:
                pass
        ...
        print(f"Epoch {epoch+1}: train_loss={avg_train_loss:.4f}, val_loss={avg_val_loss:.4f}, lr={optimizer.param_groups[0]['lr']:.2e}")
        if early_stopping_counter > 0:
            print(f"⏹️  Early stopping counter: {early_stopping_counter}/{early_stopping_patience}")
        print(f"📊 Progress: {epoch+1}/{max_epochs} epochs ({((epoch+1)/max_epochs)*100:.1f}%)")
        print(f"💾 Latest checkpoint: epoch_{epoch+1:03d}_trainer.pt")
        if (epoch + 1) % 20 == 0:
            print(f"🖼️ Validation images saved: validation_epoch_{epoch+1:03d}.png")
...
def train_real(cfg: DictConfig):
    ...
    resume_start_epoch = getattr(cfg, 'resume_start_epoch', None)
    
    if resume_checkpoint and resume_start_epoch is not None:
        print(f"🔄 Resuming training from checkpoint: {resume_checkpoint}")
        print(f"📊 Starting from epoch: {resume_start_epoch}")
        run_resume_training_real(cfg, str(resume_checkpoint), int(resume_start_epoch))
    else:
