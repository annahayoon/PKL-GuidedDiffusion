...
def infer_tiles(psf_arr: np.ndarray, input_dir: Path, out_dir: Path, rl_iters: int = 30) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    tiles = sorted(list(input_dir.glob("frame_*_patch_*.png")))
    if len(tiles) == 0:
        tiles = sorted(list(input_dir.glob("frame_*_patch_*.tif")))
    for p in tqdm(tiles, desc="RL inference"):
        wf = read_image(p).astype(np.float32)
        if wf.ndim == 3:
            wf = wf.mean(axis=-1)
        pred = richardson_lucy_restore(wf, psf_arr, num_iter=rl_iters).astype(np.float32)
...
def main():
    ap = argparse.ArgumentParser(description="Richardson–Lucy inference on WF tiles with FOV stitching")
    ...
    ap.add_argument("--run-name", default="rl_run", help="Run name for FOV previews")
    ap.add_argument("--psf", default=None, help="Optional PSF path")
    ap.add_argument("--iters", type=int, default=30, help="RL iterations")
    args = ap.parse_args()

    input_dir = Path(args.input_dir)
    out_dir = Path(args.output_dir)
    psf = PSF(args.psf).psf

    infer_tiles(psf, input_dir, out_dir, rl_iters=args.iters)
